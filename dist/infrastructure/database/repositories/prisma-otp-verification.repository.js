var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Injectable } from '@nestjs/common';
import { PrismaService } from '../prisma.service.ts';
import { OtpVerificationMapper } from '../mappers/otp-verification.mapper.ts';
let PrismaOtpVerificationRepository = class PrismaOtpVerificationRepository {
    prisma;
    constructor(prisma) {
        this.prisma = prisma;
    }
    async onModuleInit() {
        await this.prisma.$connect();
    }
    async save(otp) {
        const data = OtpVerificationMapper.toPersistence(otp);
        const result = await this.prisma.otpVerification.create({
            data,
        });
        return OtpVerificationMapper.toDomain(result);
    }
    async findLatest(identifier, type) {
        const result = await this.prisma.otpVerification.findFirst({
            where: { identifier, type: type },
            orderBy: { expires_at: 'desc' },
        });
        return OtpVerificationMapper.toDomain(result);
    }
    async delete(identifier, type) {
        await this.prisma.otpVerification.deleteMany({
            where: { identifier, type: type },
        });
    }
};
PrismaOtpVerificationRepository = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [PrismaService])
], PrismaOtpVerificationRepository);
export { PrismaOtpVerificationRepository };
//# sourceMappingURL=prisma-otp-verification.repository.js.map