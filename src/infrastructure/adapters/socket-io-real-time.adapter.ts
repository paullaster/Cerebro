...
export class SocketIoRealTimeAdapter implements IRealTimeService {
    ...
    async emitToUsers(userIds: UUIDv7[], event: string, payload: any): Promise<void> {
    ...
    async joinRoom(userId: UUIDv7, room: string): Promise<void> {
        ...
        const sockets = await this.gateway.server.in(`user:${userId.toString()}`).fetchSockets();
    ...
    async broadcast(event: string, payload: any, options?: { volatile?: boolean; namespace?: string }): Promise<void> {
        if (options?.volatile) {
