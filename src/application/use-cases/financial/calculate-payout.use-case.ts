...
@Injectable()
export class CalculatePayoutUseCase extends BaseUseCase<
  CalculatePayoutInput,
  CalculatePayoutOutput
> {
  ...
  async execute(input: CalculatePayoutInput): Promise<CalculatePayoutOutput> {
    ...
    const loanBalance =
      await this.loanRepository.getOutstandingBalance(farmerId);

    // Calculate maximum recoverable amount (Living Wage Guardrail)
    const maxRecoverable = input.collectionAmount.multiply(
      this.loanRecoveryCap,
    );
