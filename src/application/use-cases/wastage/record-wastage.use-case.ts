...
@Injectable()
export class RecordWastageUseCase extends BaseUseCase<RecordWastageInput, WastageRecord> {
    constructor(
        @Inject('ILogger') protected override readonly logger: ILogger,
        @Inject('IWastageRecordRepository') private readonly wastageRepository: IWastageRecordRepository,
    ...
    async execute(input: RecordWastageInput): Promise<WastageRecord> {
        ...
        if (input.collectionId) {
            ...
            if (collection) {
                ...
                this.logger.info('RecordWastage', 'Linked wastage to collection', { collectionId: input.collectionId });
